...
async def make_post_alert (db , payload , attachment):
    ...
    institution_category_docs = await asyncio.to_thread(
        lambda: list(
            ...
            .where(
                filter=FieldFilter(
                    "id", "==", "0229cb29-e91b-4d9c-b8e3-6307ac077579"
                )
            )
            .stream()
        )
        ...
        lambda: list(
            db.collection('institutions').where(
                filter=FieldFilter(
                    ...
                    "category_id", "==", "0229cb29-e91b-4d9c-b8e3-6307ac077579"
                )
            ).stream()
        )
    )
    ...
    await send_notification('souleyman.abdoulazis44@gmail.com' , payload.get('description'), min_location.get("name") )
